...
extip="`curl -4 ident.me`"
echo "${extip}" >  "./eXoDOS/${languagefolder}/${gamedir}/ExtIP.txt"
ipaddress=`ip -4 -o addr show up primary scope global | sed -e "s|^.*inet \(.*\)/.* brd.*|\1|"`
echo "${ipaddress}" >  "./eXoDOS/${languagefolder}/${gamedir}/ExtIP2.txt"
